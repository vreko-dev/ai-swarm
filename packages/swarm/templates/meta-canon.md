...
- [§1 Roles and Capability](#1-roles-and-capability)
  ...
  - [§1.3 Model Tier Assignments](#13-model-tier-assignments)
  - [§1.4 Role Boundaries](#14-role-boundaries)
  - [§1.5 Role Split Criteria](#15-role-split-criteria)
...
- [§3 Behavioral Primitives and Refusal Modes](#3-behavioral-primitives-and-refusal-modes)
  ...
  - [§3.6 Branch Isolation](#36-branch-isolation)
  - [§3.7 Spec Authority](#37-spec-authority)
- [§4 Dispatch and Gating](#4-dispatch-and-gating)
  ...
  - [§4.8 Authority Chain](#48-authority-chain)
  - [§4.9 Model Tier Enforcement](#49-model-tier-enforcement)
  - [§4.10 Tool Allowlist Enforcement](#410-tool-allowlist-enforcement)
- [§5 Retrospective Protocol](#5-retrospective-protocol)
  - [§5.1 Retrospective Artifacts](#51-retrospective-artifacts)
  - [§5.2 No Silent Omission](#52-no-silent-omission)
- [§6 Architecture Invariants](#6-architecture-invariants)
- [§7 Open Questions](#7-open-questions)
- [§8 Extractability Covenant](#8-extractability-covenant)
- [§9 Identifier Conventions](#9-identifier-conventions)
...
### §1.3 Model Tier Assignments

Each role is assigned a model tier. The dispatch layer enforces this — a role cannot use a higher tier than assigned without explicit human approval.
...
|---|---|---|---|
| 1 | Internal Ground Truth | `{{SWARM_DIR}}/audit-templates/internal-ground-truth.md` | Every audit, Phase 1 |
| 2 | Architecture Fence Check | `{{SWARM_DIR}}/audit-templates/architecture-fence-check.md` | Specs touching cross-package imports |
| 3 | Ratchet Baseline Capture | `{{SWARM_DIR}}/audit-templates/ratchet-baseline-capture.md` | Before any ratchet-affecting change |
| 4 | Caller/Callee Impact | `{{SWARM_DIR}}/audit-templates/caller-callee-impact.md` | Any method signature change |
| 5 | Test Inventory | `{{SWARM_DIR}}/audit-templates/test-inventory.md` | Specs adding or removing tests |
| 6 | External Dependency Assumption Enumeration | `{{SWARM_DIR}}/audit-templates/external-dependency-enumeration.md` | Phase 2 audit; thin prompts; any spec depending on external system behavior |
...
- Branch isolation check fails — no agent works directly on `{{BRANCH_MAIN}}` or `{{BRANCH_DEV}}`
...
All agents run in git worktrees. No agent works directly on `{{BRANCH_MAIN}}` or `{{BRANCH_DEV}}`. The first action of every agent is a branch isolation check via `branch-check.sh`.

### §3.7 Spec Authority

The spec is the definition of done. If the spec's verification commands pass, the work is complete. If they fail, the work is incomplete. No judgment calls.
...
- Worktrees are ALWAYS created off `{{BRANCH_DEV}}`: `git worktree add .worktrees/<spec-id> -b task/<spec-id> {{BRANCH_DEV}}`
- PRs ALWAYS target `{{BRANCH_DEV}}`: `gh pr create --base {{BRANCH_DEV}} --head task/<spec-id>`
- `git checkout -b` in the main working tree is FORBIDDEN — creates untracked feature branches outside the swarm state model
- Only the release manager opens PRs targeting `{{BRANCH_MAIN}}` (`{{BRANCH_DEV}}` → `{{BRANCH_MAIN}}` promotion)

Full agent rules: `{{SWARM_DIR}}/docs/reference/branch-model.md` § "Agent Rules"

Worktrees not in `current.json.worktrees[]` but present in `git worktree list` are orphaned. Conductor investigates before proceeding. If orphan content is confirmed on `{{BRANCH_DEV}}`, remove the worktree. If content is not on `{{BRANCH_DEV}}`, surface to operator.

### §4.5 State Authority

`{{SWARM_DIR}}/state/current.json` is the authoritative record of swarm state. Conductor reads it at session start without exception. `swarm-state.sh` commands are the only sanctioned write interface — no manual JSON edits.
...
The following precedence chain governs conflicts between instruction sources. Higher in the chain wins conflicts.
...
### §4.9 Model Tier Enforcement

Each role is assigned a model tier (see §1.3). The dispatch layer enforces this — a role cannot use a higher tier than assigned without explicit human approval.

### §4.10 Tool Allowlist Enforcement

Each role has a restricted tool set (see §1.2). The dispatch layer enforces this — a role cannot use tools outside its allowlist.
...
If an artifact has zero items, the block is still emitted with empty arrays or zero counts. Silent omission is a protocol violation.
...
| Q6 | When does external research split from Auditor/Spec Writer into a dedicated role? | **Wait for retrospective evidence.** Trigger criteria per §1.5: persistent latency > 2×, materially different failure modes, 3+ swarm runs confirming. |
...
3. Project-specific ratchet values, package counts, and SHAs belong in `{{SWARM_DIR}}/state/current.json` — not in canon
4. Role prompts in `{{SWARM_DIR}}/agents/` are generic; project-specific extensions go in separate amendment files
5. Audit templates use placeholder package names that the operator fills in at instantiation time
6. All project-specific values are injected at runtime via token replacement from `SwarmContext`
